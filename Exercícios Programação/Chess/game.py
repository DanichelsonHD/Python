...
class Game:
    ...
    def undo_move(self):
        ...
        # Restaura peça capturada normalmente, se houver
        if captured_piece:
            self.board.grid[row_idx][col_idx] = captured_piece
        
        # Restaura peão capturado via en passant (estava numa casa diferente do destino)
        en_passant_piece = move_data.get('en_passant_piece')
        en_passant_pos = move_data.get('en_passant_pos')
        if en_passant_piece and en_passant_pos:
            ep_col, ep_row = self.board.convert_position(en_passant_pos)
            self.board.grid[ep_row][ep_col] = en_passant_piece
            en_passant_piece.column = en_passant_pos[0]
            en_passant_piece.row = en_passant_pos[1]
        ...
        # Restaura last_double_move de todos os peões adversários
        opponent_pawn_states = move_data.get('opponent_pawn_double_move_states', {})
        for pawn, had_double_move in opponent_pawn_states.items():
            pawn.last_double_move = had_double_move
    ...
    def play_move(self, move_input: str):
        ...
        # Salva o estado last_double_move dos peões adversários ANTES de zerar
        # (necessário para restaurar corretamente no undo)
        opponent_color = 'black' if self.current_turn == 'white' else 'white'
        opponent_pawns = self._find_pieces_by_type_and_color('P', opponent_color)
        opponent_pawn_double_move_states = {
            pawn: pawn.last_double_move for pawn in opponent_pawns
        }
        ...
        self.move_stack.append({
            ...
            'opponent_pawn_double_move_states': opponent_pawn_double_move_states,
        })
